...
<%
	...
	if ( ( ( String ) request.getMethod() ).compareTo( "POST" )==0 ) {
		...
		vCheck = request.getParameter("cChecked");
...
<script type="text/javascript">
	...
	function validarCheckbox( vForm, vAction, bot ) {
		vContI = 0;
		vCedula = "";
		valorCheck = "";
		for ( var i = 0; i < window.document.vForm.elements.length; i++) {
		...
		window.document.vForm.boton.value = bot;
		if ( vContI<=0 ) {
...
<table  width="60%" border="0" align="center">
	<tr>
		...
		<div align="left">
		<p><label>Desde <input type="text" name="desde" id="desde" size="10" readonly="readonly" value="<%=vDesde%>">
		...
		<td width="31%" class="style9"><label>Hasta <input type="text" name="hasta" id="hasta" size="10" readonly="readonly" value="<%=vHasta%>">
		...
		<td width="15%"><input type="button" align="middle" class="style9" name="consultar" id="consultar" value="Consultar" OnClick="validar( window.document.vForm, 'BuscadorCedulasServlet' );">
		</td>
		<td width="22%">  
			<input type="button" align="middle" class="style9" name="limpiar" id="limpiar" value="Limpiar" OnClick="window.location.href='CedulasEvaluarView.jsp'">
		</td>
	</tr>
</table>
<p align="center">&nbsp;</p>
...
%>
		...
		if (vCantI > 1) {
			if (1 < vFila) {
		%>
				<a href="javascript:fPaginacion( 'BuscadorCedulasServlet', <%=vRowNumTotal%>, <%=(vRowNumIni-vMaxRow)%>, <%=vMaxRow%>, <%=vFila-1%> );">
		&lt;Anterior</a> 
			<%
			} else {
				%>
			...
			%> 
				<a href="javascript:fPaginacion( 'BuscadorCedulasServlet', <%=vRowNumTotal%>, <%=(((vContI*vMaxRow)-vMaxRow)+1)%>, <%=vMaxRow%>, <%=vContI%> );"><%=vContI%></a> 
			<%
			}
			if (vCantI>vFila) {
			%> 
...
<table width="200" border="0" align="center">
	<tr>
		<td>
		<div align="center"><label> <input type="button" name="Aprobar" id="Aprobar" class="style9" value="Aprobar" Onclick="validarCheckbox( window.document.vForm, 'ModificarCedulasNuevasServlet', 'Aprobar' ); window.document.vForm.boton.value='Aprobar';"> </label></div>
		</td>
		<td>
		<div align="center"><label><input type="button" name="Rechazar" id="Rechazar" class="style9" value="Rechazar" Onclick="validarCheckbox( window.document.vForm, 'ModificarCedulasNuevasServlet' ); window.document.vForm.boton.value='Rechazar';"> </label></div>
		</td>
	</tr>
</table>
</form>
